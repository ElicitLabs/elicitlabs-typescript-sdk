# Elicit Client TypeScript API Library
...
This library provides convenient access to the Elicit Client REST API from server-side TypeScript or JavaScript.
...
1. Via the `ELICIT_CLIENT_LOG` environment variable
...
const client = new ElicitClient({
  logger: logger.child({ name: 'ElicitClient' }),
...
const client = new ElicitClient({ fetch });
```
...
```ts
import ElicitClient from 'npm:elicit-labs';
